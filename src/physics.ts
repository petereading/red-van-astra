...
export class Physics {
  ...
  teleport(s:DriveState){this.player.setTranslation({x:s.x,y:1.5,z:s.z},true);this.player.setLinvel({x:0,y:0,z:0},true);this.player.setRotation({x:0,y:Math.sin(-s.heading/2),z:0,w:Math.cos(-s.heading/2)},true);}
  reset(){for(const c of this.props)c.setEnabled(true);}
  ...
  step(state:DriveState,input:Controls,health:number,offroad:boolean,onHit:(target:HitTarget,speed:number)=>void):DriveState{
    ...
    this.player.setLinvel({x:Math.sin(next.heading)*next.speed,y:0,z:-Math.cos(next.heading)*next.speed},true);
    this.player.setRotation({x:0,y:Math.sin(-next.heading/2),z:0,w:Math.cos(-next.heading/2)},true);
    this.world.step(this.queue);
    this.queue.drainCollisionEvents((h1,h2,started)=>{if(!started)return;const other=h1===this.playerCollider.handle?h2:h2===this.playerCollider.handle?h1:undefined;if(other===undefined)return;const target=this.targets.get(other);if(!target)return;onHit(target,Math.abs(next.speed));if(target.obstacle?.group&&Math.abs(next.speed)>3){target.obstacle.knocked=true;this.world.getCollider(other)?.setEnabled(false);}});
    ...
    return{x:p.x,z:p.z,heading:next.heading,speed};
  }
}
