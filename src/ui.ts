...
export class UI {
  ...
  ready(){(this.el('start')as HTMLButtonElement).disabled=false;this.text('start-text','開車！');}
  ...
  results(stats:Stats,seed:number,reason:string){const sc=scoreRun(stats);this.setMode('results');this.text('grade',sc.grade);this.el('grade').dataset.grade=sc.grade;this.text('result-title',stats.completed?'準時到站！':reason);this.text('result-subtitle',stats.completed?'今日這一轉，辛苦晒。':'再開一轉，路況又會不同。');this.text('result-score',sc.total.toLocaleString());
    ...
    if(stats.completed){try{const best=JSON.parse(localStorage.getItem('redvan-best')||'null');if(!best||sc.total>best.score)localStorage.setItem('redvan-best',JSON.stringify({score:sc.total,grade:sc.grade}));}catch{}}this.updateBest();}
