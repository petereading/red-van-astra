...
export class StreetLighting {
  ...
  constructor(public scene:T.Scene,public city:City){
    ...
    for(const lamp of city.lamps){const pool=new T.Mesh(new T.PlaneGeometry(15,21),mat);pool.rotation.x=-Math.PI/2;pool.position.set(lamp.position.x,.235,lamp.position.z);pool.visible=false;scene.add(pool);this.pools.push(pool);}
  }
  ...
  update(dt:number,x:number,z:number,heading:number){
    ...
    if(this.night&&this.clock<=0){this.clock=.2;const lamps=this.city.lamps.map((g,i)=>({g,i,d:(g.position.x-x)**2+(g.position.z-z)**2})).sort((a,b)=>a.d-b.d);
      ...
      this.pools.forEach((p,i)=>{const lamp=this.city.lamps[i];p.visible=this.night&&Math.hypot(lamp.position.x-x,lamp.position.z-z)<135;p.position.set(lamp.position.x,.235,lamp.position.z);});
    }
